...
import { SeniorRoomRecommendPanel } from './components/SeniorRoomRecommendPanel'
import { ageLabelFromDobString, mergeParsedWithGuru } from './lib/id-guru-fields'
import { ageYearsFromDobString, isGuestUnderMinimumAge, isIdExpired } from './lib/id-age'
import { guestQualifiesForSeniorRecommend } from './lib/senior-room-recommend'
...
import { DualPmsPanel } from './components/DualPmsPanel'
...
/** After this idle period, auto save+clear unsaved guest draft (with countdown warning). */
const GUEST_IDLE_SAVE_CLEAR_MS = 2 * 60 * 1000
const GUEST_IDLE_SAVE_CLEAR_COUNTDOWN_S = 30
/** Clear saved ID form + key/folio stay after this idle period (or after To PMS). */
const SESSION_IDLE_CLEAR_MS = 2 * 60 * 1000
...
function App() {
  ...
  const [underageManagerEmail, setUnderageManagerEmail] = useState('')
  const [underageManagerPassword, setUnderageManagerPassword] = useState('')
  const [underageApproved, setUnderageApproved] = useState(false)
  const [dnrActive, setDnrActive] = useState(false)
  const [dnrCheckBusy, setDnrCheckBusy] = useState(false)
  const [seniorRecommendBusy, setSeniorRecommendBusy] = useState(false)
  const [seniorRecommendResult, setSeniorRecommendResult] = useState<SeniorRoomRecommendResult | null>(
    null,
  )
  const [seniorRecommendError, setSeniorRecommendError] = useState<string | null>(null)
  const [seniorRoomRequests, setSeniorRoomRequests] = useState<SeniorRoomTypeRequest[]>([
    { roomType: 'NK1', count: 1 },
  ])
  ...
  const keyHistoryRef = useRef<KeyHistoryRow[]>([])
  ...
  const keyIdleTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  const [ezeeGroupSwitchBusy, setEzeeGroupSwitchBusy] = useState(false)
  const [findGuestBusy, setFindGuestBusy] = useState(false)
  const [exportBusy, setExportBusy] = useState(false)
  const [twainScanBusy, setTwainScanBusy] = useState(false)
  const [twainScanError, setTwainScanError] = useState<string | null>(null)
  const twainScanTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const [hotelPolicyBusy, setHotelPolicyBusy] = useState(false)
  ...
  /** True while the form holds a history-loaded entry (not a fresh scan). */
  const fromHistoryRef = useRef(false)
  /** True once "To PMS" was clicked for the current form entry. */
  const toPmsClickedRef = useRef(false)
  /** When To PMS last succeeded — start idle-clear deadline from here too. */
  const toPmsCompletedAtRef = useRef<number | null>(null)
  ...
  /** `front` / `back` = one side captured; `complete` = both sides + OCR applied. */
  const idScanPassRef = useRef<'idle' | 'front' | 'back' | 'complete'>('idle')
  ...
  type WorkspaceTab = 'id' | 'history' | 'payment' | 'signature' | 'dualpms' | 'keys' | 'key'
  const [activeTab, setActiveTab] = useState<WorkspaceTab>('id')
  const activeTabRef = useRef<WorkspaceTab>('id')
  const keyBusyRef = useRef(false)
  const readCardBusyRef = useRef(false)
  const cancelCardBusyRef = useRef(false)
  const autoSaveCountdownRef = useRef<number | null>(null)
  const idFormHasDraftRef = useRef(false)
  const regCardSigningRef = useRef(false)
  const clearKeyEncodeTabSessionRef = useRef<() => Promise<void>>(async () => {})
  const runGuestSessionIdleClearRef = useRef<() => Promise<void>>(async () => {})
  ...
  // Auto-refresh SynXis balance every 3 minutes so stale balance never blocks key encoding.
  useEffect(() => {
    const t = window.setInterval(async () => {
      try {
        const res = (await chrome.runtime.sendMessage({ type: 'REFRESH_BALANCE' })) as {
          ok?: boolean
  ...
  const clearIdScan = useCallback((opts?: { clearReservation?: boolean }) => {
    ...
    toPmsClickedRef.current = false
    toPmsCompletedAtRef.current = null
    setUnderageApproved(false)
    ...
    if (opts?.clearReservation !== false) {
  ...
  const seniorRecommendEnabled = state?.seniorRecommendEnabled ?? true
  const seniorRecommendAge = state?.seniorRecommendAge ?? 50
  ...
  const showSeniorRecommend = useMemo(
    () =>
      guestQualifiesForSeniorRecommend(guestAgeYears, {
        seniorRecommendEnabled,
        seniorRecommendAge,
        seniorPreferredFloors: [1],
        seniorPreferredRoomList: '',
      }),
    [guestAgeYears, seniorRecommendEnabled, seniorRecommendAge],
  )
  const idExpired = useMemo(
    () => isIdExpired(parsed.expiryDate),
    [parsed.expiryDate],
  )

  useEffect(() => {
    setUnderageApproved(false)
    setSeniorRecommendResult(null)
    setSeniorRecommendError(null)
  }, [parsed.dateOfBirth])

  useEffect(() => {
    if (!showSeniorRecommend || !state?.auth.signedIn) {
      setSeniorRecommendBusy(false)
      if (!showSeniorRecommend) {
        setSeniorRecommendResult(null)
        setSeniorRecommendError(null)
      }
      return
    ...
    void (async () => {
      try {
        const res = (await chrome.runtime.sendMessage({
          ...
          roomRequests: seniorRoomRequests,
        })) as ExtensionResponse
        if (cancelled) return
        if (res.ok && res.seniorRoomRecommendations) {
          setSeniorRecommendResult(res.seniorRoomRecommendations)
          setSeniorRecommendError(null)
        } else if (!res.ok) {
          setSeniorRecommendResult(null)
          setSeniorRecommendError((res as { error: string }).error)
        }
      } catch {
        if (!cancelled) {
          setSeniorRecommendResult(null)
          setSeniorRecommendError('Could not reach extension service')
        }
      } finally {
        if (!cancelled) setSeniorRecommendBusy(false)
      }
    })()
    return () => {
      cancelled = true
    }
  }, [showSeniorRecommend, state?.auth.signedIn, parsed.dateOfBirth, seniorRoomRequests])
    ...
    async (entry: IdScanLogEntry) => {
      ...
      try {
        ...
        if (frontB64 || backB64) {
          ...
          idScanPassRef.current =
            frontB64 && backB64 ? 'complete' : backB64 && !frontB64 ? 'back' : 'front'
  ...
  const applyScanSidePreview = useCallback(async (side: IdScanSide, imageB64: string) => {
    const b64 = normalizeScanBase64(imageB64)
    if (!b64) return

    const prev = scanImagesRef.current
    if (
      idScanPassRef.current === side &&
      ((side === 'front' && prev?.front === b64 && !prev?.back?.trim()) ||
        (side === 'back' && prev?.back === b64 && !prev?.front?.trim()))
    ) {
      return
    }
    ...
    if (
      ...
      prev?.front?.trim() &&
      prev?.back?.trim() &&
      ((side === 'front' && prev.front === b64) || (side === 'back' && prev.back === b64))
    ) {
      return
    }
    ...
    const nextImages =
      side === 'front'
        ? { front: b64, back: prev?.back?.trim() ? prev.back : null }
        : { front: prev?.front?.trim() ? prev.front : '', back: b64 }
  ...
  const applyScanFrontPreview = useCallback(
    async (frontB64: string) => applyScanSidePreview('front', frontB64),
    [applyScanSidePreview],
  )
  ...
  const applyNativeIdScan = useCallback(
    async (m: NativeIdScanBroadcast) => {
      ...
      if (!isCompleteTwoSidedScan(front, back)) {
        if (back && !front) {
          await applyScanSidePreview('back', back)
        } else if (front) {
          await applyScanSidePreview('front', front)
        }
        return
      }
      ...
      // New guest — clear key encode tab (folio, history, encoder session) and show ID tab.
      await clearKeyEncodeTabSessionRef.current()
      toPmsCompletedAtRef.current = null

      const phase = await readScanPhase()
      if ((phase === 'front' || phase === 'back') && idScanPassRef.current === phase) {
        frontScanStartedAtRef.current = null
      }
      ...
      try {
        ...
        if (!isGuestUnderMinimumAge(m.parsed.dateOfBirth, minimumCheckInAge)) {
          showChromeNotification(
            'FrontDesk Nexus',
            'ID scan received — enter phone if needed, then To PMS to send and save.',
          )
        }
      } catch (err) {
        console.warn(
  ...
  const applyScanSidePreviewRef = useRef(applyScanSidePreview)
  useEffect(() => {
    applyScanSidePreviewRef.current = applyScanSidePreview
  }, [applyScanSidePreview])
  ...
  const applyScanSideResult = useCallback(
    async (d: ScanSideBroadcast) => {
      let b64 = normalizeScanBase64(d.imageBase64)
      if (!b64 && d.imagesInStorage) {
        for (let attempt = 0; attempt < 8; attempt++) {
          const stored = await readScanImagesFromStorage()
          b64 = d.side === 'front' ? (stored?.front ?? '') : (stored?.back ?? '')
          if (b64) break
          await new Promise((r) => window.setTimeout(r, 40 * (attempt + 1)))
        }
      }
      if (!b64) {
        console.warn(`[FrontDesk Nexus] ${d.side} scan — no image in message or storage`)
        return
      }
      await applyScanSidePreview(d.side, b64)
    },
    [applyScanSidePreview],
  )
  ...
  const applyScanFrontResult = useCallback(
    async (d: ScanFrontBroadcast) => {
      ...
      await applyScanSidePreview('front', b64)
    },
    [applyScanSidePreview],
  )
  ...
  useEffect(() => {
    const onRuntimeMessage = (msg: unknown) => {
      ...
      if (m.type === 'FDN_SCAN_SIDE_RESULT') {
        void applyScanSideResult(msg as ScanSideBroadcast)
        return
      }
      ...
      if (m.type === 'FDN_TWAIN_SCAN_ERROR') {
        if (twainScanTimeoutRef.current !== null) {
          clearTimeout(twainScanTimeoutRef.current)
          twainScanTimeoutRef.current = null
        }
        setTwainScanBusy(false)
        setTwainScanError((m as { type: string; message: string }).message)
        return
      }
      if (m.type === 'REG_CARD_SIGNATURE_COMPLETE') {
        returnToBlankIdScanTabRef.current()
      }
    }
    ...
    const onStorageChanged = (
      ...
      const phaseChanged =
        changes[FDN_SCAN_PHASE_KEY]?.newValue === 'front' ||
        changes[FDN_SCAN_PHASE_KEY]?.newValue === 'back'

      if (imageChanged || phaseChanged) {
        void readScanImagesFromStorage().then(async (stored) => {
          if (!stored?.front && !stored?.back) return
          ...
          if (!isCompleteTwoSidedScan(front, back)) {
            if (back && !front) {
              await applyScanSidePreviewRef.current('back', back)
            } else if (front) {
              await applyScanSidePreviewRef.current('front', front)
            }
            return
          }
      ...
      if (changes[FDN_REG_CARD_SIGNING_ACTIVE_KEY]) {
        regCardSigningRef.current = changes[FDN_REG_CARD_SIGNING_ACTIVE_KEY].newValue === true
      }
    }
  ...
  }, [applyScanFrontResult, applyScanSideResult, refresh])

  // Restore in-progress side preview when the side panel opens mid two-pass scan.
  useEffect(() => {
    void (async () => {
      ...
      if (!stored?.front && !stored?.back) return
      if (scanImagesRef.current?.front || scanImagesRef.current?.back) return
      ...
      if (stored.back && !stored.front) {
        await applyScanSidePreviewRef.current('back', stored.back)
      } else if (stored.front) {
        await applyScanSidePreviewRef.current('front', stored.front)
      } else if (phase === 'back' && stored.back) {
        await applyScanSidePreviewRef.current('back', stored.back)
      }
    })()
  }, [])
  ...
  async function onOpenHotelPolicy() {
    setHotelPolicyBusy(true)
    setFormError(null)
    try {
      const res = (await chrome.runtime.sendMessage({ type: 'OPEN_HOTEL_POLICY' })) as ExtensionResponse
      if (!res.ok) {
        setFormError('error' in res ? res.error : 'Could not open hotel policy on guest display.')
      }
    } catch (e) {
      setFormError(e instanceof Error ? e.message : 'Could not open hotel policy on guest display.')
    } finally {
      setHotelPolicyBusy(false)
    }
  }
  ...
  async function onTransferToPms() {
    ...
    try {
      ...
      const saveRes = await persistGuestIdScanRef.current({
        clearAfter: false,
        silent: false,
        notification: 'Guest sent to PMS and saved — clears after 2 min idle or next scan.',
      })
      if (!saveRes.ok) {
        const err = saveRes.error ?? 'PMS filled but ID save failed.'
        setFormError(err)
        void chrome.notifications.create({
          type: 'basic',
      ...
      toPmsCompletedAtRef.current = Date.now()
      lastPanelInteractionRef.current = Date.now()
      lastFormInteractionRef.current = Date.now()
    } catch (e) {
  ...
  useEffect(() => {
    readCardBusyRef.current = readCardBusy
  }, [readCardBusy])

  useEffect(() => {
    cancelCardBusyRef.current = cancelCardBusy
  }, [cancelCardBusy])

  useEffect(() => {
    autoSaveCountdownRef.current = autoSaveCountdown
  }, [autoSaveCountdown])

  useEffect(() => {
    idFormHasDraftRef.current = hasUnsavedGuestDraft(
      idDetail,
      phone,
      emailGuest,
      parsed,
  ...
  useEffect(() => {
    canIdleSaveClearRef.current = () => {
      // History entry without To PMS click: don't auto-save or auto-clear.
      if (fromHistoryRef.current && !toPmsClickedRef.current) return false
      if (!idFormHasDraftRef.current) return false
      return validateRequiredGuestFields(idDetail, phone) === null
    }
  }, [idDetail, phone])

  const returnToBlankIdScanTab = useCallback(() => {
    setActiveTab('id')
    clearIdScan({ clearReservation: false })
    setAutoSaveCountdown(null)
    lastFormInteractionRef.current = Date.now()
    lastPanelInteractionRef.current = Date.now()
  }, [clearIdScan])

  const returnToBlankIdScanTabRef = useRef(returnToBlankIdScanTab)
  useEffect(() => {
    returnToBlankIdScanTabRef.current = returnToBlankIdScanTab
  }, [returnToBlankIdScanTab])

  useEffect(() => {
    void chrome.storage.local.get(FDN_REG_CARD_SIGNING_ACTIVE_KEY).then((stored) => {
      regCardSigningRef.current = stored[FDN_REG_CARD_SIGNING_ACTIVE_KEY] === true
    })
  }, [])
  ...
  // Idle Save & Clear: after 2 min with no form interaction, warn 30s then save + clear (if required fields are valid).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (autoSaveCountdown <= 0) {
      void persistGuestIdScanRef.current({
        ...
        silent: false,
        notification:
          'Guest check-in saved — form cleared after 2 minutes of inactivity.',
      }).then(() => {
        setAutoSaveCountdown(null)
        lastFormInteractionRef.current = Date.now()
      })
  ...
  // After 2 min idle (or 2 min after To PMS), clear ID form + key/folio stay and return to ID tab.
  useEffect(() => {
    const handle = window.setInterval(() => {
      if (regCardSigningRef.current) return
      if (keyBusyRef.current || readCardBusyRef.current || cancelCardBusyRef.current) return
      if (autoSaveCountdownRef.current !== null) return
      const scanPass = idScanPassRef.current
      if (scanPass === 'front' || scanPass === 'back') return

      const now = Date.now()
      const idleMs = now - Math.max(lastPanelInteractionRef.current, lastFormInteractionRef.current)
      const toPmsAt = toPmsCompletedAtRef.current
      const toPmsIdle = toPmsAt ? now - toPmsAt : 0

      const hasReservation = !!stateRef.current?.reservation?.confirmationNumber
      const hasKeyHistory = (keyHistoryRef.current?.length ?? 0) > 0
      const hasIdWork =
        scanPass === 'complete' ||
        idFormHasDraftRef.current ||
        toPmsAt != null

      if (!hasIdWork && !hasReservation && !hasKeyHistory) return

      const triggerByIdle = idleMs >= SESSION_IDLE_CLEAR_MS
      const triggerByToPms = toPmsAt != null && toPmsIdle >= SESSION_IDLE_CLEAR_MS

      if (triggerByIdle || triggerByToPms) {
        const unsavedDraft = idFormHasDraftRef.current && toPmsAt == null
        if (triggerByToPms || !unsavedDraft) {
          void runGuestSessionIdleClearRef.current()
        } else if (hasReservation || hasKeyHistory) {
          void clearKeyEncodeTabSessionRef.current()
        }
      }
    }, 15_000)
    return () => clearInterval(handle)
  ...
  async function onVerifyUnderageManager(e: React.FormEvent) {
    e.preventDefault()
    setBusy(true)
    setFormError(null)
    try {
      const res = (await chrome.runtime.sendMessage({
        type: 'VERIFY_MANAGER',
        email: underageManagerEmail,
        password: underageManagerPassword,
      })) as { ok: boolean; error?: string }
      if (!res.ok) {
        setFormError(res.error ?? 'Verification failed')
        return
      }
      setUnderageApproved(true)
      setShowUnderageModal(false)
      showChromeNotification('FrontDesk Nexus', 'Manager authorized — To PMS is now available for this guest.')
    } finally {
      setBusy(false)
    }
  }
  ...
  /** Encoder session only — idle timeout; does not clear loaded guest stay. */
  function resetKeyEncoderSessionState() {
    setSessionCheckinTime(null)
    setSessionNextSerial(2)
    setKeyNotice(null)
    setKeyBlocks([])
    ...
    setOverridePinInput('')
    setOverridePinError(null)
    if (keyIdleTimerRef.current) {
      clearTimeout(keyIdleTimerRef.current)
      keyIdleTimerRef.current = null
    }
  }

  function resetKeyIdleTimer() {
    if (keyIdleTimerRef.current) clearTimeout(keyIdleTimerRef.current)
    keyIdleTimerRef.current = setTimeout(() => {
      resetKeyEncoderSessionState()
    }, SESSION_IDLE_CLEAR_MS)
  }

  /** Clear loaded stay, folio, last card read, encoder session, and key history. */
  const clearKeyEncodeTabSession = useCallback(async () => {
    resetKeyEncoderSessionState()
    setReadCardResult(null)
    setKeyHistory([])
    lastLoadedConfRef.current = null
    try {
      const resp = (await chrome.runtime.sendMessage({ type: 'CLEAR_RESERVATION' })) as ExtensionResponse
      if (resp?.ok && 'state' in resp && resp.state) {
        setState(resp.state)
      } else {
        await refresh()
      }
  ...
  useEffect(() => {
    clearKeyEncodeTabSessionRef.current = clearKeyEncodeTabSession
  }, [clearKeyEncodeTabSession])

  /** Clear ID form + key/folio session after idle or post–To PMS deadline. */
  const runGuestSessionIdleClear = useCallback(async () => {
    const hasReservation = !!stateRef.current?.reservation?.confirmationNumber
    const hasKeyHistory = (keyHistoryRef.current?.length ?? 0) > 0
    const hasIdWork =
      idScanPassRef.current === 'complete' ||
    ...
    if (hasReservation || hasKeyHistory) {
      await clearKeyEncodeTabSession()
    }
    if (hasIdWork) {
      clearIdScan({ clearReservation: false })
    }
    ...
    setAutoSaveCountdown(null)
    setActiveTab('id')
  }, [clearIdScan, clearKeyEncodeTabSession])

  useEffect(() => {
    runGuestSessionIdleClearRef.current = runGuestSessionIdleClear
  }, [runGuestSessionIdleClear])

  /** Clear loaded stay, last card read, encoder session, and key history for a fresh key tab. */
  async function onClearKeyEncoder() {
    toPmsCompletedAtRef.current = null
    await clearKeyEncodeTabSession()
  }
  ...
  async function onSelectEzeeGroupMember(index: number) {
    if (!state || index === state.ezeeGroupActiveIndex) return
    setKeyNotice(null)
    setEzeeGroupSwitchBusy(true)
    try {
      const resp = (await chrome.runtime.sendMessage({
        type: 'SELECT_EZEE_GROUP_MEMBER',
        index,
      })) as ExtensionResponse
      if (resp?.ok && 'state' in resp && resp.state) {
        setState(resp.state)
        ...
        const errMsg =
          'error' in resp && typeof resp.error === 'string'
            ? resp.error
            : 'Could not switch group room'
        setKeyNotice(errMsg)
      }
    } catch {
      setKeyNotice('Could not switch group room')
    } finally {
      setEzeeGroupSwitchBusy(false)
    }
  }
  ...
  const ezeeGroupMembers = state.ezeeGroupMembers ?? []
  const ezeeGroupActiveIndex = state.ezeeGroupActiveIndex ?? 0
  ...
  const saveGuestReady = Boolean(
    idDetail.firstName?.trim() && idDetail.lastName?.trim() && phone.trim(),
  )
  const findGuestReady = Boolean(idDetail.lastName?.trim())
  const guestActionsDisabled = busy || !state.auth.signedIn || !saveGuestReady

  const toPmsBlockedByUnderage = guestUnderage && minimumCheckInAge > 0 && !underageApproved
  const transferTooltip = !state.auth.signedIn
    ...
    : !saveGuestReady
      ...
      : toPmsBlockedByUnderage
        ? 'Manager must authorize underage check-in before sending to PMS'
        : 'Send to PMS and save — form clears after 2 min idle or next scan'
  ...
  const workspaceTabs: {
    ...
    Icon: typeof IconId
  }[] = [
      {
        ...
        status: scanImages || manualEntry || saveGuestReady || findGuestReady ? 'ready' : 'idle',
        Icon: IconId,
      },
      {
        id: 'history',
  ...
  return (
    <div className="fdn-root">
      <PanelHeader
        ...
        idScannerSources={hw.idScannerSources ?? []}
        thalesConnected={hw.thalesConnected ?? false}
        selectedScanner={state.selectedScanner ?? 'thales'}
        rfidEncoder={hw.rfid_encoder}
        idCheckedAgo={idCheckedAgo}
        keyCheckedAgo={keyCheckedAgo}
        rfidCheckBusy={rfidCheckBusy}
        hotelPolicyBusy={hotelPolicyBusy}
        onOpenHotelPolicy={() => void onOpenHotelPolicy()}
        onLogout={() => void onLogout()}
        onRefreshId={() => void refresh()}
        onCheckKey={() => void onCheckRfid()}
        onSelectScanner={(s) => {
          void chrome.runtime.sendMessage({ type: 'SELECT_SCANNER', scanner: s }).then((res) => {
            ...
            >
              ...
              {autoSaveCountdown !== null && (
                <div className="fdn-autosave-banner" role="status">
                  <span>Save &amp; Clear in {autoSaveCountdown}s (2 min idle)…</span>
                  <button
                    type="button"
              ...
              {hasIdScanContext ? (
                <IdScanAlerts
                  ...
                  expired={idExpired}
                  underageApproved={underageApproved}
                  onRequestUnderageApproval={() => {
                    setUnderageManagerEmail('')
                    setUnderageManagerPassword('')
                    setFormError(null)
              ...
              {hasIdScanContext && showSeniorRecommend && guestAgeYears !== null ? (
                <div className="fdn-id-alerts" role="status" aria-live="polite">
                  <SeniorRoomRecommendPanel
                    guestAgeYears={guestAgeYears}
                    busy={seniorRecommendBusy}
                    result={seniorRecommendResult}
                    error={seniorRecommendError}
                    requests={seniorRoomRequests}
                    onRequestsChange={setSeniorRoomRequests}
                  />
                </div>
              ) : null}
                        ...
                        <div className="fdn-twain-scan-wrap">
                          ...
                          <span
                            className={[
                              'fdn-twain-status',
                              twainScanBusy
                                ? 'fdn-twain-status--waiting'
                                : hw.id_scanner === 'connected'
                                  ? 'fdn-twain-status--ready'
                                  : 'fdn-twain-status--offline',
                            ].join(' ')}
                          >
                            {twainScanBusy
                              ? '● Insert card…'
                          ...
                          <span className="fdn-id-preview-slot__placeholder">
                            {manualEntry
                              ...
                              : scanPreviewUrls.back
                                ? 'Flip card — scan front…'
                                : 'Waiting for front scan…'}
                          </span>
                        )}
                      </div>
                    </div>
              ...
              <div className="fdn-grid fdn-grid--idguru fdn-grid--dense">
                ...
                <div className="fdn-field fdn-field--full">
                  <LabelText className="fdn-field__label">Phone &amp; country</LabelText>
                  <p className="fdn-zip-hint">Required for Save and To PMS — not for Find Guest.</p>
              ...
              <div className="fdn-panel__footer">
                <button
                  ...
                  disabled={findGuestBusy || !findGuestReady}
                  title={
                    !findGuestReady
                      ? 'Enter last name from the ID scan'
                      : 'Search PMS by last name — phone and email not required'
                  }
                ...
                <button
                  ...
                  disabled={guestActionsDisabled || toPmsBlockedByUnderage}
          ...
          {activeTab === 'keys' ? (
            <KeysOperationsPanel
              ...
              frontDeskWriteGranted={
                state.auth.role === 'front_desk' &&
                !!state.frontDeskKeysWriteAccessUntil &&
                new Date(state.frontDeskKeysWriteAccessUntil).getTime() > Date.now()
              }
              defaultCheckoutTime={state.defaultCheckoutTime}
              frontDeskDefaultKeyDays={state.frontDeskDefaultKeyDays}
              frontDeskDefaultKeyDaysEnabled={state.frontDeskDefaultKeyDaysEnabled}
              onOpenIdScanEntry={(entry) => void loadHistoryEntryIntoScanner(entry)}
            />
          ) : null}
          ...
          {activeTab === 'key' ? (
            <section className="fdn-panel fdn-panel--key">
              <div className="fdn-key-tab__stay-toolbar">
                ...
                {res?.confirmationNumber && !res?.roomNumber ? (
                  <button
                    type="button"
                    className="fdn-btn fdn-btn--secondary"
                    title="Clear loaded stay, last card read, and key encoder session"
                    onClick={() => void onClearKeyEncoder()}
                  >
                    Clear
                  </button>
                ) : null}
              </div>
              {ezeeGroupMembers.length > 1 ? (
                <div className="fdn-ezee-group-picker" role="radiogroup" aria-label="Group reservation rooms">
                  <p className="fdn-ezee-group-picker__label">
                    Group stay · {ezeeGroupMembers.length} rooms
                    {ezeeGroupSwitchBusy ? ' · loading…' : ''}
                  </p>
                  <div className="fdn-ezee-group-picker__options">
                    {ezeeGroupMembers.map((m, i) => (
                      <label
                        key={m.confirmationNumber}
                        className={`fdn-ezee-group-picker__option${
                          i === ezeeGroupActiveIndex ? ' fdn-ezee-group-picker__option--active' : ''
                        }`}
                      >
                        <input
                          type="radio"
                          name="fdn-ezee-group-member"
                          checked={i === ezeeGroupActiveIndex}
                          disabled={ezeeGroupSwitchBusy}
                          onChange={() => void onSelectEzeeGroupMember(i)}
                        />
                        <span className="fdn-ezee-group-picker__text">
                          Rm {m.roomNumber ?? '?'} · {m.confirmationNumber}
                        </span>
                      </label>
                    ))}
                  </div>
              ...
              <div className="fdn-key-verify" style={{ marginTop: 8 }}>
                <p className="fdn-help" style={{ marginTop: 0 }}>
                  Verify a key card (room, dates, serial) without loading a reservation.
                </p>
      ...
      {showUnderageModal && (
        <div className="fdn-modal-backdrop" role="dialog" aria-modal="true">
          <div className="fdn-modal">
            <h3 className="fdn-h3">Manager authorization required</h3>
            <p className="fdn-help">
              Guest is under the minimum check-in age ({minimumCheckInAge}). A manager or admin must
              authorize before sending to PMS.
            </p>
            {formError ? (
              <p className="fdn-form-error" role="alert">
                {formError}
            ...
            <form className="fdn-form" onSubmit={(e) => void onVerifyUnderageManager(e)}>
              <label className="fdn-label">
                Manager email
                <input
                  className="fdn-input"
                  ...
                  autoComplete="username"
                  required
                  value={underageManagerEmail}
                  onChange={(e) => setUnderageManagerEmail(e.target.value)}
                />
              </label>
              <label className="fdn-label">
                Password
                <input
                  ...
                  autoComplete="current-password"
                  required
                  value={underageManagerPassword}
                  onChange={(e) => setUnderageManagerPassword(e.target.value)}
                />
              </label>
              <div className="fdn-actions">
                <button type="submit" className="fdn-btn fdn-btn--primary" disabled={busy}>
