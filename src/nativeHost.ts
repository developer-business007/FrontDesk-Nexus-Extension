...
import { isCompleteTwoSidedScan } from './lib/scan-image-storage'
...
/** Called when the two-pass scan thread pushes a single-side image (other side pending). */
export type NativeHostScanSideCallback = (side: 'front' | 'back', imageBase64: string) => void

/** @deprecated Use {@link NativeHostScanSideCallback}. */
  ...
  const connect = () => {
    ...
    port.onMessage.addListener((raw: unknown) => {
      ...
      if (isAutoScanResult(raw)) {
        ...
        const detectedSide =
          raw.detected_side === 'back' ? 'back' : raw.detected_side === 'front' ? 'front' : null
        const sessionPending = raw.session_pending === true

        if ((sessionPending || (frontB64 && !backB64) || (!frontB64 && backB64)) && !isCompleteTwoSidedScan(frontB64, backB64)) {
          const side: 'front' | 'back' =
            detectedSide ??
            (frontB64 && !backB64 ? 'front' : 'back')
          const imageB64 = side === 'front' ? frontB64 : backB64
          console.log(
            `${LOG} AUTO_SCAN_RESULT ${side} pass only — preview (${imageB64.length} chars), waiting for other side`,
          )
          if (imageB64 && onScanSide) onScanSide(side, imageB64)
          return
        }
      ...
      if (raw.type === 'SCAN_FRONT_RESULT') {
        ...
        if (imageFront && onScanSide) {
          onScanSide('front', imageFront)
        }
        return
      }
