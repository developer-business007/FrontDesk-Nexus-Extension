import {
  extractEzeeGroupReservationList,
...
} from '../lib/ezee-gr-card-modal'
import {
  armRegCardViewerWatch,
  clearStaleReportFrames,
} from '../lib/ezee-reg-card-report'
import {
  applyEzeeGroupMemberToFields,
  resolveActiveEzeeGroupIndex,
} from '../lib/ezee-group'
...
function buildExtractPayload(): EzeeExtractPayload {
  ...
  const groupMembers = extractEzeeGroupReservationList(document)
  const hasGroup = groupMembers.length > 1
  const drawerOpen = isEzeeGuestDrawerOpen(document)
  ...
  if (!isCompleteEzeeGuestScrape(fields)) {
    console.warn('[FDN eZee] EZEE_EXTRACT_NOW: scrape failed', probeEzeeDrawer(document))
    return {
      ok: false,
      error: 'Could not read complete guest data — open Guest Details or the Arrivals guest drawer.',
  ...
  if (hasGroup) {
    activeGroupIndex = resolveActiveEzeeGroupIndex(groupMembers, fields)
    fields = applyEzeeGroupMemberToFields(fields, groupMembers[activeGroupIndex])
  }
...
async function runDetection(): Promise<void> {
  ...
  const balanceHint = (fields.balance ?? '').replace(/\s/g, '')
  const dedupeKey = `${conf}|${roomHint}|${statusHint}|${balanceHint}`
  const now = Date.now()
...
chrome.runtime.onMessage.addListener(
  (
    ...
    sendResponse: (r: InjectResult | ReturnType<typeof buildExtractPayload> | { ok: boolean; error?: string } | { balance: string | null }) => void,
  ) => {
    ...
    if (message?.type === 'GET_CURRENT_BALANCE') {
      ...
      sendResponse({ balance: fields?.balance ?? null })
      return
    }
...
let singleResRegCardTimer = 0

function getRegCardFallbackConf(): string {
  return lastDedupeKey ? lastDedupeKey.split('|')[0]! : ''
}

function openRegCardSignPopup(reportUrl: string, confirmation: string): void {
  console.log('[FDN eZee] Opening sign popup for', confirmation, reportUrl.slice(0, 120))
  void chrome.runtime.sendMessage({
    type: 'EZEE_OPEN_REG_CARD',
    ezeeReportUrl: reportUrl,
    confirmation,
  }).catch(() => { /* extension may not be listening */ })
}

/** Arm watcher — eZee opens "Print Guest Registration Card" viewer modal; we read its iframe URL. */
function armRegCardCapture(confirmation: string): void {
  const conf = confirmation.trim()
  if (!conf) {
    console.warn('[FDN eZee] Reg card: no confirmation — aborting')
    return
  }

  window.clearTimeout(singleResRegCardTimer)
  singleResRegCardTimer = 0

  clearStaleReportFrames()
  armRegCardViewerWatch(conf, openRegCardSignPopup)
}

installGrCardModalPrintListener(getRegCardFallbackConf, (conf) => {
  void chrome.runtime.sendMessage({
    type: 'EZEE_PRINT_BASIC_CARD_CLICKED',
    confirmation: conf,
  }).catch(() => { /* ignore */ })
  armRegCardCapture(conf)
})
...
document.addEventListener(
  ...
  (event) => {
    ...
    const fallbackConf = getRegCardFallbackConf()
    console.log('[FDN eZee] Print Guest Registration Card clicked | confirmation:', fallbackConf || '(unknown)')

    void chrome.runtime.sendMessage({
      type: 'EZEE_PRINT_BASIC_CARD_CLICKED',
      confirmation: fallbackConf,
    }).catch(() => { /* extension may not be listening */ })

    window.clearTimeout(singleResRegCardTimer)

    // Group: GR options Print re-arms with the selected reservation.
    // Single: viewer modal opens directly — arm unless options modal appears.
    singleResRegCardTimer = window.setTimeout(() => {
      singleResRegCardTimer = 0
      if (findGrCardOptionsModal()) return
      armRegCardCapture(fallbackConf)
    }, 800)
  },
  true,
)
