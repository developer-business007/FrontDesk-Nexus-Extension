...
// ── Registration card sign popup (capture phase pierces stopPropagation) ───────

const REG_CARD_API =
  'https://sph.synxis.com/pms-web-ui/service/v1/guest-mgt/guest-stay-record/registration-card'

function regCardClickSource(el: HTMLElement): string | null {
  if (el.tagName === 'LI' && el.textContent?.trim() === 'Print Basic Registration Card') {
    return 'Print Basic Registration Card'
  }
  if (el.id === 'print-reg-card-btn' || el.closest('#print-reg-card-btn')) {
    return 'Check In PRINT'
  }
  return null
}

async function openRegCardSignPopup(source: string): Promise<void> {
  const confirmation = extractConfirmationFromDocument(document)
  console.log(`[FDN SPH] ${source} clicked | confirmation:`, confirmation)
  if (!confirmation) {
    console.warn('[FDN SPH] No confirmation number found, aborting PDF fetch')
    return
  }
  try {
    console.log('[FDN SPH] Fetching registration card PDF...')
    const res = await fetch(REG_CARD_API, {
      method: 'POST',
    ...
    if (!res.ok) {
      console.error('[FDN SPH] PDF fetch failed:', res.status, res.statusText)
      return
    }
    const data = (await res.json()) as { reportData?: string; status?: string; successful?: boolean }
    if (!data.reportData) {
      console.error('[FDN SPH] No reportData in response')
      return
    }
...
document.addEventListener(
  'click',
  (event) => {
    const source = regCardClickSource(event.target as HTMLElement)
    if (!source) return
    void openRegCardSignPopup(source)
  },
