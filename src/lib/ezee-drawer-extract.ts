import type { EzeeGuestDisplay, EzeeGroupMember, ReservationSnapshot } from '../shared/pms-types'
...
export function isEzeeBookingDetailsTabActive(doc: Document): boolean {
  return /booking\s*details/i.test(getEzeeActiveTabLabel(doc))
}
...
export function isEzeeGuestScrapeAllowed(doc: Document): boolean {
  ...
  if (isEzeeReservationDetailShell(doc)) {
    return isEzeeGuestDetailsTabActive(doc) || isEzeeBookingDetailsTabActive(doc)
  }
...
function extractEzeeFieldsFromRoot(body: HTMLElement): EzeeScrapeFields | null {
  const fullText = (body.innerText ?? body.textContent ?? '').replace(/\u00a0/g, ' ')
...
export function extractEzeeScrapeFields(doc: Document): EzeeScrapeFields | null {
  ...
  return extractEzeeFieldsFromRoot(body)
}

/**
 * Guest drawer first; on reservation detail (Booking / Guest Details) scrape the active tab pane.
 */
export function extractEzeeScrapeFieldsFromPage(doc: Document): EzeeScrapeFields | null {
  if (!isEzeeGuestScrapeAllowed(doc)) return null

  const fromDrawer = extractEzeeScrapeFields(doc)
  if (fromDrawer) return fromDrawer

  if (!isEzeeReservationDetailShell(doc)) return null

  const pane =
    doc.querySelector<HTMLElement>('.ant-tabs-tabpane-active') ??
    doc.querySelector<HTMLElement>('.ant-layout-content')
  if (!pane) return null

  return extractEzeeFieldsFromRoot(pane)
}

const GROUP_SUB_RES_RE = /\b(\d{3,}-\d+)\b/

function compactEzeeText(s: string): string {
  return s.replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim()
}

function extractStandaloneRoomLine(text: string, confirmation: string): string | null {
  const lines = text.split(/\n/).map((line) => line.trim()).filter(Boolean)
  const confIdx = lines.findIndex((line) => line.includes(confirmation))
  const start = confIdx >= 0 ? Math.max(0, confIdx - 1) : 0
  const end = confIdx >= 0 ? Math.min(lines.length, confIdx + 4) : lines.length
...
function extractRoomFromGroupCardText(text: string, confirmation: string): string | null {
  const rm =
    text.match(/\bRm\.?\s*#?\s*(\d{3,4})\b/i) ??
    text.match(/\bRoom\s*#?\s*(\d{3,4})\b/i)
  if (rm?.[1]) return rm[1]

  const standalone = extractStandaloneRoomLine(text, confirmation)
  if (standalone) return standalone

  const idx = text.indexOf(confirmation)
  const tail = idx >= 0 ? text.slice(idx + confirmation.length) : text
  const nums = tail.match(/\b(\d{3,4})\b/g) ?? []
  for (const n of nums) {
    if (n.length >= 3 && n.length <= 4) return n
  }
  return null
}

function parseEzeeGroupMemberCard(el: HTMLElement): EzeeGroupMember | null {
  let cardEl: HTMLElement | null = el
  for (let depth = 0; depth < 5 && cardEl; depth++) {
    const text = compactEzeeText(cardEl.innerText ?? cardEl.textContent ?? '')
    if (text.length < 12 || text.length > 900) {
      cardEl = cardEl.parentElement
      continue
    }

    const confMatch = text.match(GROUP_SUB_RES_RE)
    if (!confMatch?.[1]) {
      cardEl = cardEl.parentElement
      continue
    }
    const confirmationNumber = normalizeEzeeReservationNumber(confMatch[1])
    if (!confirmationNumber?.includes('-')) return null

    const dates = text.match(/\d{2}-\d{2}-\d{4}/g) ?? []
    const statusMatch = text.match(
      /\b(Arrived|Confirmed|Reserved|Checked\s*In|Due\s*In|No\s*Show|Cancelled|Checked\s*Out)\b/i,
    )
    const guestMatch = text.match(
      /(?:Mr\.|Mrs\.|Ms\.|Dr\.|Miss)\s+[A-Z][\w'.-]*(?:\s+[A-Z][\w'.-]*)+/,
    )

    return {
      confirmationNumber,
      roomNumber: extractRoomFromGroupCardText(text, confirmationNumber),
      guestName: guestMatch?.[0]?.trim() ?? null,
      arrivalDateRaw: dates[0] ?? null,
      departureDateRaw: dates[1] ?? null,
...
function findGroupListContainer(doc: Document): HTMLElement | null {
  const walker = doc.createTreeWalker(doc.body, NodeFilter.SHOW_TEXT)
  let node: Node | null
  while ((node = walker.nextNode())) {
    const raw = node.textContent ?? ''
    if (!/group\s*reservation\s*list/i.test(raw)) continue
    let el = node.parentElement
    for (let depth = 0; depth < 10 && el; depth++) {
      const cards = collectGroupMemberCards(el)
...
function collectGroupMemberCards(root: HTMLElement): EzeeGroupMember[] {
  const leafEls: HTMLElement[] = []
  for (const el of root.querySelectorAll<HTMLElement>('*')) {
    const text = compactEzeeText(el.innerText ?? '')
    if (!GROUP_SUB_RES_RE.test(text) || text.length > 650 || text.length < 12) continue
    const childHit = [...el.children].some((child) => {
      const ct = compactEzeeText((child as HTMLElement).innerText ?? '')
      return GROUP_SUB_RES_RE.test(ct) && ct.length < text.length
    })
    if (childHit) continue
    leafEls.push(el)
  }

  const byConf = new Map<string, EzeeGroupMember>()
  for (const el of leafEls) {
    const member = parseEzeeGroupMemberCard(el)
    if (!member) continue
    byConf.set(member.confirmationNumber, member)
  }

  return [...byConf.values()].sort((a, b) =>
    a.confirmationNumber.localeCompare(b.confirmationNumber, undefined, { numeric: true }),
  )
}

/** Scrape eZee "Group Reservation List" drawer when present (2+ sub-reservations). */
export function extractEzeeGroupReservationList(doc: Document): EzeeGroupMember[] {
  if (!/group\s*reservation\s*list/i.test(doc.body?.innerText ?? '')) return []

  const container = findGroupListContainer(doc)
  if (!container) return []

  const members = collectGroupMemberCards(container)
  if (members.length < 2) return []

  const prefix = members[0].confirmationNumber.split('-')[0]
  if (!prefix || !members.every((m) => m.confirmationNumber.startsWith(`${prefix}-`))) {
    return []
  }
