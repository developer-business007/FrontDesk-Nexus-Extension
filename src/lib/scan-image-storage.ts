...
export type IdScanPhase = 'front' | 'back' | 'complete'
export type IdScanSide = 'front' | 'back'
...
export async function readScanPhase(): Promise<IdScanPhase | null> {
  ...
  return v === 'front' || v === 'back' || v === 'complete' ? v : null
}

/** Write one side during a two-pass scan without clearing the other slot. */
export async function writeScanSideToStorage(side: IdScanSide, b64: string): Promise<void> {
  const norm = normalizeScanBase64(b64)
  if (!norm) return
  const key = side === 'front' ? FDN_SCAN_IMAGE_FRONT_KEY : FDN_SCAN_IMAGE_BACK_KEY
  const otherKey = side === 'front' ? FDN_SCAN_IMAGE_BACK_KEY : FDN_SCAN_IMAGE_FRONT_KEY
  const stored = await chrome.storage.local.get([key, otherKey])
  await chrome.storage.local.set({
    [key]: norm,
    [otherKey]: typeof stored[otherKey] === 'string' ? stored[otherKey] : '',
    [FDN_SCAN_PHASE_KEY]: side,
  })
}
