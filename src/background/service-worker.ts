...
import {
  applyEzeeGroupMemberToReservation,
  displayFromEzeeReservationDetail,
  formatEzeeMoneyDisplay,
  memberFromEzeeReservationDetail,
  reservationFromEzeeReservationDetail,
  type EzeeReservationDetail,
} from '../lib/ezee-group'
...
import { localDateRangeToUtcIso, localDateString } from '../lib/local-date'
import { buildSignatureLogFromRows } from '../lib/signature-log'
import {
  createRoomBlockSw,
  fetchKeyBoardData,
  fetchKeyLedger,
  fetchSeniorRoomRecommendations,
  keysAddGuestSw,
  keysMoveRoomSw,
  keysRemoveGuestSw,
  keysWriteAuthorized,
...
import { fetchPmsBoardData, triggerPmsSync } from '../lib/pms-board-sw'
import { requestPmsHousekeeping } from '../lib/pms-housekeeping-sw'
import { isCompletePhoneForLookup } from '../lib/phone-lookup'
import {
  isCompleteTwoSidedScan,
  writeScanImagesToStorage,
  writeScanPhase,
  writeScanSideToStorage,
} from '../lib/scan-image-storage'
import { createExtensionSupabase } from '../lib/supabase-factory'
import { uploadBytesToB2 } from '../lib/b2-storage'
...
// instantly without waiting for a new ping + DEVICE_STATUS round-trip.
let cachedIdScanner: 'connected' | 'disconnected' = 'disconnected'
let cachedIdScannerSources: string[] = []
let cachedThalesConnected = false
let hwStatusFreshAt = 0
const HW_STATUS_TTL_MS = 10_000
...
let ezeeGroupMembers: EzeeGroupMember[] = []
let ezeeGroupActiveIndex = 0
let ezeeGroupBaseSnapshot: ReservationSnapshot | null = null
let ezeeGroupBaseDisplay: EzeeGuestDisplay | null = null
...
// ── SynXis cookie backup (extension → Supabase app_settings.synxis_session) ────
const SYNXIS_COOKIE_SAVE_DEDUPE_MS = 5 * 60 * 1000
let synxisCookieSavedAt = 0
let synxisCookieSaveInFlight = false

function isSynxisUrl(u: string | null | undefined): boolean {
  if (!u) return false
  return /^https:\/\/controlcenter-p2\.synxis\.com\//i.test(u) || /^https:\/\/sph\.synxis\.com\//i.test(u)
}

function cookieHeaderFromCookies(cookies: chrome.cookies.Cookie[]): string {
  // Cookie header order generally doesn’t matter, but keep it stable to reduce churn.
  const parts = cookies
...
async function captureAndSaveSynxisCookie(reason: 'tab_complete' | 'manual' = 'tab_complete'): Promise<void> {
  if (synxisCookieSaveInFlight) return
  const now = Date.now()
  if (reason !== 'manual' && now - synxisCookieSavedAt < SYNXIS_COOKIE_SAVE_DEDUPE_MS) return

  synxisCookieSaveInFlight = true
  try {
    const client = getClient()
    const { data: sess } = await client.auth.getSession()
    if (!sess.session) return // not signed into portal yet

    const [cc, sph] = await Promise.all([
      chrome.cookies.getAll({ domain: 'controlcenter-p2.synxis.com' }),
      chrome.cookies.getAll({ domain: 'sph.synxis.com' }),
    ])
    const cookieHeader = cookieHeaderFromCookies([...(cc ?? []), ...(sph ?? [])])
    if (!cookieHeader) return

    const { error } = await client.functions.invoke('save-synxis-credentials', {
      body: { action: 'saveSession', cookieHeader, source: 'extension' },
    })
    if (error) {
      console.warn('[FDN SW] SynXis cookie save failed:', error.message)
      return
    }
    synxisCookieSavedAt = now
  } catch (e) {
    console.warn('[FDN SW] SynXis cookie capture error:', e instanceof Error ? e.message : e)
  } finally {
    synxisCookieSaveInFlight = false
  }
}
...
/** Built-in hotel policy guest-facing window. */
let hotelPolicyWindowId: number | null = null

chrome.windows.onRemoved.addListener((windowId) => {
  if (hotelPolicyWindowId === windowId) {
    hotelPolicyWindowId = null
  }
  for (const [conf, wid] of ezeeRegCardWindowByConf) {
    if (wid === windowId) {
      ezeeRegCardWindowByConf.delete(conf)
      void chrome.storage.local.remove(FDN_REG_CARD_SIGNING_ACTIVE_KEY)
      break
    }
  }
})
...
async function runRfidMakeKey(msg: RfidMakeKeyMessage): Promise<ExtensionResponse | Record<string, unknown>> {
  ...
  if (!isOverride) {
    ...
    // Live balance refresh — always fetch fresh before gate check (fail open on error)
    if (reservation?.pms === 'synxis' && reservation.confirmationNumber) {
      try {
        const accounting = await fetchSynxisAccountingSummary(reservation.confirmationNumber)
        if (accounting && accounting.estimatedRemaining !== null) {
          reservation = { ...reservation, dueAmount: String(accounting.estimatedRemaining) }
        }
      } catch { /* fail open */ }
    } else if (reservation?.pms === 'ezee' && reservation.confirmationNumber) {
      try {
        const billing = await fetchEzeeAccountingSummary(reservation.confirmationNumber)
        if (billing !== null) {
          reservation = { ...reservation, dueAmount: String(billing.dueAmount) }
        }
      } catch { /* fail open */ }
    }
...
type EzeeBillingSummary = { dueAmount: number; totalCharges: number; paidAmount: number }

/**
 * Fetches eZee balance via Supabase Edge Function `ezee-billing` (auth code stays server-side).
 * Requires a signed-in session. Returns null on any failure (fail open).
 */
async function fetchEzeeAccountingSummary(bookingId: string): Promise<EzeeBillingSummary | null> {
  ...
  try {
    ...
    if (!sessionData.session) {
      console.warn('[FDN SW] ezee-billing: no signed-in session')
      return null
    }

    const { data, error } = await client.functions.invoke('ezee-billing', {
      body: { bookingId: id },
    })
    if (error) {
      console.warn('[FDN SW] ezee-billing invoke error', error)
      return null
    }
    ...
    if (typeof row.error === 'string' && row.error.trim()) {
      console.warn('[FDN SW] ezee-billing returned error', row.error)
      return null
    }

    const dueAmount = Number(row.dueAmount)
    const totalCharges = Number(row.totalCharges)
    const paidAmount = Number(row.paidAmount)
    if (!Number.isFinite(dueAmount)) return null

    const summary: EzeeBillingSummary = {
      dueAmount,
      totalCharges: Number.isFinite(totalCharges) ? totalCharges : 0,
      paidAmount: Number.isFinite(paidAmount) ? paidAmount : 0,
    }
    console.info('[FDN SW] eZee billing summary', { bookingId: id, ...summary })
    return summary
  } catch (e) {
    console.warn('[FDN SW] ezee-billing fetch failed', e)
    return null
  }
}
...
async function completeSynxisReservationFromConfirmation(
  ...
  if (sess.session) {
    ...
    dbSaved = ur.ok
    if (ur.ok) reservationId = ur.id
  }

  const conf = apiRes.payload.confirmationNumber ?? confirmation

  if (sess.session && reservationId && conf) {
    await linkIdScansToReservation(client, {
      reservationId,
      confirmationNumber: conf,
      guestName: apiRes.payload.guestName ?? null,
      roomNumber: apiRes.payload.roomNumber ?? null,
      checkOutDate: apiRes.payload.checkOutDate ?? null,
      pmsSource: apiRes.payload.pms,
    })
  }
...
async function persistEzeeGroupState(): Promise<void> {
  if (ezeeGroupMembers.length > 1) {
    await chrome.storage.local.set({
      fdn_ezee_group_members: ezeeGroupMembers,
      fdn_ezee_group_active_index: ezeeGroupActiveIndex,
      fdn_ezee_group_base_snapshot: ezeeGroupBaseSnapshot,
      fdn_ezee_group_base_display: ezeeGroupBaseDisplay,
    })
    return
  }
  await chrome.storage.local.remove([
    'fdn_ezee_group_members',
    'fdn_ezee_group_active_index',
    'fdn_ezee_group_base_snapshot',
    'fdn_ezee_group_base_display',
  ])
}

function clearEzeeGroupState(): void {
  ezeeGroupMembers = []
  ezeeGroupActiveIndex = 0
  ezeeGroupBaseSnapshot = null
  ezeeGroupBaseDisplay = null
}

async function fetchEzeeReservationDetailFromApi(
  bookingId: string,
): Promise<EzeeReservationDetail | null> {
  const id = bookingId.trim()
  if (!id) return null
  try {
    const client = getClient()
    const { data: sessionData } = await client.auth.getSession()
    if (!sessionData.session) {
      console.warn('[FDN SW] ezee-reservation-detail: no signed-in session')
      return null
    }

    const { data, error } = await client.functions.invoke('ezee-reservation-detail', {
      body: { bookingId: id },
    })
    if (error) {
      console.warn('[FDN SW] ezee-reservation-detail invoke error', error)
      return null
    }
    if (!data || typeof data !== 'object' || Array.isArray(data)) return null
    ...
    const row = data as Record<string, unknown>
    if (typeof row.error === 'string' && row.error.trim()) {
      console.warn('[FDN SW] ezee-reservation-detail returned error', row.error)
      return null
    }
    ...
    const totalCharges = Number(row.totalCharges)
    const paidAmount = Number(row.paidAmount)
    if (!Number.isFinite(dueAmount) || !Number.isFinite(totalCharges) || !Number.isFinite(paidAmount)) {
      return null
    }

    const detail: EzeeReservationDetail = {
      confirmationNumber: typeof row.confirmationNumber === 'string' ? row.confirmationNumber : id,
      roomNumber: typeof row.roomNumber === 'string' ? row.roomNumber : null,
      checkInDate: typeof row.checkInDate === 'string' ? row.checkInDate : null,
      checkOutDate: typeof row.checkOutDate === 'string' ? row.checkOutDate : null,
    ...
    console.info('[FDN SW] eZee reservation detail', { bookingId: id, room: detail.roomNumber })
    return detail
  } catch (e) {
    console.warn('[FDN SW] ezee-reservation-detail fetch failed', e)
    return null
  }
}

async function applyEzeeBillingToLoadedGuest(bookingId: string): Promise<void> {
  const billing = await fetchEzeeAccountingSummary(bookingId)
  if (billing === null || !reservation) return
  const balanceDisplay = formatEzeeMoneyDisplay(billing.dueAmount)
  reservation = { ...reservation, dueAmount: String(billing.dueAmount) }
  if (ezeeGuestDisplay) {
    ezeeGuestDisplay = { ...ezeeGuestDisplay, balance: balanceDisplay }
  }
}

async function applyEzeeReservationDetailToLoadedGuest(
  ...
  reservation = reservationFromEzeeReservationDetail(baseSnapshot, detail, tabUrl, loadedAt)
  ezeeGuestDisplay = displayFromEzeeReservationDetail(baseDisplay, detail)
}

async function enrichEzeeGroupMembersFromApi(): Promise<void> {
  if (ezeeGroupMembers.length <= 1) return
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return

  const results = await Promise.all(
    ezeeGroupMembers.map(async (member, index) => {
      const detail = await fetchEzeeReservationDetailFromApi(member.confirmationNumber.trim())
      return { index, detail }
    }),
  )
  ...
  for (const { index, detail } of results) {
    if (!detail) continue
    ezeeGroupMembers[index] = memberFromEzeeReservationDetail(ezeeGroupMembers[index], detail)
    changed = true
  }
  if (changed) void persistEzeeGroupState()
}
...
async function completeEzeeReservationFromSnapshot(
  ...
  const members = groupMembers && groupMembers.length > 1 ? groupMembers : []
  if (members.length > 1) {
    ezeeGroupMembers = members
    ezeeGroupActiveIndex = Math.min(
      Math.max(activeGroupIndex ?? 0, 0),
      members.length - 1,
    )
    ezeeGroupBaseSnapshot = { ...snapshot }
    ezeeGroupBaseDisplay = { ...guestDisplay }
  } else {
    clearEzeeGroupState()
  }
  ...
  void chrome.storage.local.set({
    ...
    fdn_ezee_guest_display: ezeeGuestDisplay,
  })
  void persistEzeeGroupState()
...
async function selectEzeeGroupMember(index: number): Promise<ExtensionResponse> {
  ...
  if (index < 0 || index >= ezeeGroupMembers.length) {
    return { ok: false, error: 'Invalid group room selection' }
  }

  const baseSnapshot = ezeeGroupBaseSnapshot
  const baseDisplay = ezeeGroupBaseDisplay
  if (!baseSnapshot || !baseDisplay) {
    return { ok: false, error: 'Group reservation context missing — refresh stay from eZee' }
  }

  ezeeGroupActiveIndex = index
  const member = ezeeGroupMembers[index]
  const bookingId = member.confirmationNumber.trim()
  const tabUrl = reservation?.pageUrl ?? baseSnapshot.pageUrl
  const loadedAt = new Date().toISOString()
  ...
  if (detail) {
    reservation = reservationFromEzeeReservationDetail(baseSnapshot, detail, tabUrl, loadedAt)
    ezeeGuestDisplay = displayFromEzeeReservationDetail(baseDisplay, detail)
    ezeeGroupMembers[index] = memberFromEzeeReservationDetail(member, detail)
  } else {
    const merged = applyEzeeGroupMemberToReservation(
      baseSnapshot,
      baseDisplay,
      member,
      tabUrl,
    ...
    reservation = merged.snapshot
    ezeeGuestDisplay = merged.guestDisplay
    await applyEzeeBillingToLoadedGuest(bookingId)
  }
...
const RESERVATION_LINK_LOOKBACK_DAYS = 30

function stayDateForDb(value: string | null | undefined): string | null {
  return normalizeHotelStayDate(null, value)
}
...
/** Recent reservations whose guest name matches (DB only — no PMS API). */
async function findReservationsByGuestName(
  client: SupabaseClient,
  guestName: string,
  limit = 20,
): Promise<ReservationLinkRow[]> {
  const tokens = [...nameTokens(guestName)].sort((a, b) => b.length - a.length)
  if (!tokens.length) return []

  const cutoff = new Date(Date.now() - RESERVATION_LINK_LOOKBACK_DAYS * 86_400_000)
    .toISOString()
    .slice(0, 10)
  ...
  let query = client
    .from('reservations')
    .select(
      'id, confirmation_number, guest_name, guest_profile_id, room_number, check_in_date, check_out_date, updated_at',
    )
    .gte('check_out_date', cutoff)
    .order('updated_at', { ascending: false })
    .limit(80)

  const { data, error } = await query.ilike('guest_name', `%${primary}%`)
  if (error) {
    console.warn('[FDN SW] findReservationsByGuestName', error.message)
    return []
  }

  return (data ?? [])
    .filter((r) => namesMatch(guestName, r.guest_name as string | null))
    .slice(0, limit)
    .map((r) => ({
      id: r.id as string,
      confirmation_number: r.confirmation_number as string,
      guest_name: (r.guest_name as string | null) ?? null,
      guest_profile_id: (r.guest_profile_id as string | null) ?? null,
...
async function nameFromIdScanPii(
  piiEncrypted: unknown,
): Promise<string | null> {
  if (!piiEncrypted || typeof piiEncrypted !== 'object') return null
  try {
    const pii = await decryptJson<{
      fullName?: string
      idGuru?: { firstName?: string; lastName?: string }
    }>(piiEncrypted as EncryptedPayload)
    const fromGuru = [pii.idGuru?.firstName, pii.idGuru?.lastName]
      .filter((p) => typeof p === 'string' && p.trim())
      .join(' ')
      .trim()
    return fromGuru || pii.fullName?.trim() || null
  } catch {
    return null
  }
}

/** Link id_scans + guest_profile to a reservation (conf #, name match on orphans). */
async function linkIdScansToReservation(
  client: SupabaseClient,
  args: {
  ...
  if (!conf || !args.reservationId) return

  const patch: Record<string, unknown> = {
    reservation_id: args.reservationId,
    confirmation_number: conf,
  }
  if (args.roomNumber != null) patch.room_number = args.roomNumber
  if (args.checkOutDate != null) patch.check_out_date = stayDateForDb(args.checkOutDate)

  const { error: exactErr } = await client
    .from('id_scans')
    .update(patch)
    .eq('confirmation_number', conf)
  if (exactErr) console.warn('[FDN SW] id_scans link (exact conf)', exactErr.message)

  if (args.guestName) {
    const cutoff = new Date(Date.now() - RESERVATION_LINK_LOOKBACK_DAYS * 86_400_000).toISOString()
    const { data: orphans, error: orphanErr } = await client
      .from('id_scans')
      .select('id, confirmation_number, pii_encrypted, guest_profile_id')
      .like('confirmation_number', 'NO-RES-%')
      .gte('scanned_at', cutoff)
      .limit(60)

    if (orphanErr) {
      console.warn('[FDN SW] id_scans orphan query', orphanErr.message)
    } else {
      for (const row of orphans ?? []) {
        const scanName = await nameFromIdScanPii(row.pii_encrypted)
        if (!scanName || !namesMatch(scanName, args.guestName)) continue
        const { error: linkErr } = await client
          .from('id_scans')
          .update(patch)
          .eq('id', row.id)
        if (linkErr) console.warn('[FDN SW] id_scans orphan link', linkErr.message)
        else {
          console.info('[FDN SW] Linked orphan ID scan to reservation', {
            scanId: row.id,
            confirmation: conf,
          })
        }
        const profileId = row.guest_profile_id as string | null
        if (profileId) {
          await client
            .from('reservations')
            .update({ guest_profile_id: profileId })
            .eq('id', args.reservationId)
            .eq('pms_source', args.pmsSource)
        }
      }
    }
  }
...
async function upsertReservationSnapshot(
  ...
  const row = {
    ...
    check_in_date: stayDateForDb(snap.checkInDate),
    check_out_date: stayDateForDb(snap.checkOutDate),
    ...
    if (connected) {
      try {
        ...
        if (Array.isArray(statusResp?.twain_sources)) {
          cachedIdScannerSources = statusResp.twain_sources as string[]
        }
        cachedThalesConnected = Boolean(statusResp?.thales_dll_load_ok)
      } catch {
        // DEVICE_STATUS is best-effort — keep previous values on error
      }
    } else {
      cachedIdScannerSources = []
      cachedThalesConnected = false
    }
  } catch {
    cachedIdScanner = 'disconnected'
    cachedIdScannerSources = []
    cachedThalesConnected = false
    idScannerStatusCheckedAt = Date.now()
  }
  hwStatusFreshAt = Date.now()
}

async function buildHardwareStatus(): Promise<ExtensionState['hardware']> {
  // Refresh cache if stale; otherwise return cached values instantly.
  if (Date.now() - hwStatusFreshAt >= HW_STATUS_TTL_MS) {
    await refreshHardwareCache()
  }

  // Check RFID once on startup only (rfidStatusCheckedAt === 0).
  ...
  return {
    id_scanner: cachedIdScanner,
    spectral_payout: 'disconnected',
    rfid_encoder: rfidConnected,
    idScannerSources: cachedIdScannerSources,
    thalesConnected: cachedThalesConnected,
  }
}
...
async function restorePersistedReservationState(): Promise<void> {
  ...
  if (ezeeGroupMembers.length === 0 && Array.isArray(stored.fdn_ezee_group_members)) {
    const members = stored.fdn_ezee_group_members as EzeeGroupMember[]
    if (members.length > 1 && reservation?.pms === 'ezee') {
      ezeeGroupMembers = members
      const idx = Number(stored.fdn_ezee_group_active_index)
      ezeeGroupActiveIndex = Number.isFinite(idx)
        ? Math.min(Math.max(idx, 0), members.length - 1)
        : 0
      ezeeGroupBaseSnapshot = (stored.fdn_ezee_group_base_snapshot as ReservationSnapshot | null) ?? null
      ezeeGroupBaseDisplay = (stored.fdn_ezee_group_base_display as EzeeGuestDisplay | null) ?? null
    }
  }
}
...
async function getState(): Promise<ExtensionState> {
  ...
  return {
    ...
    frontDeskKeysWriteAccessUntil: hotelSettings.frontDeskKeysWriteAccessUntil,
    frontDeskDefaultKeyDays: hotelSettings.frontDeskDefaultKeyDays,
    frontDeskDefaultKeyDaysEnabled: hotelSettings.frontDeskDefaultKeyDaysEnabled,
    defaultCheckoutTime: hotelSettings.defaultCheckoutTime,
    seniorRecommendEnabled: hotelSettings.seniorRecommendEnabled,
...
async function saveIdScan(args: {
  ...
  if (isUpdate) {
    ...
    if (snap?.confirmationNumber && idName && !reservationBound) {
      console.warn('[FDN SW] Name mismatch — active PMS guest not bound', { idName, pmsName })
    }

    if (reservationBound && snap?.confirmationNumber) {
      conf = snap.confirmationNumber
      const ur = await upsertReservationSnapshot(client, snap)
      if (ur.ok) resId = ur.id
      else {
        const { data, error } = await client
          .from('reservations')
          .select('id')
      ...
      const matches = await findReservationsByGuestName(client, idName, 5)
      if (matches.length > 0) {
        const best = matches[0]!
        reservationBound = true
        conf = best.confirmation_number
        resId = best.id
        console.info('[FDN SW] ID scan linked to reservation by name', {
          idName,
          confirmation: conf,
        })
      } else {
  ...
  try {
    if (args.imageFrontBase64) {
      ...
      try {
        ...
        console.warn('Front image upload', e instanceof Error ? e.message : e)
      }
    }
    if (args.imageBackBase64) {
      ...
      try {
        ...
        console.warn('Back image upload', e instanceof Error ? e.message : e)
      }
    }
  } catch (e) {
...
async function saveSignature(args: {
  ...
  try {
    ...
    lastError = msg
    return { ok: false, error: `PDF upload failed: ${msg}` }
  }
...
async function notifyRegCardSignatureComplete(): Promise<void> {
  await chrome.storage.local.remove(FDN_REG_CARD_SIGNING_ACTIVE_KEY)
  try {
...
/** Guest-facing hotel policy: 2nd display + chrome fullscreen (F11-like, no window chrome). */
async function moveHotelPolicyToGuestDisplay(windowId: number): Promise<void> {
  try {
    const displays = await chrome.system.display.getInfo()
    const second = displays.find((d) => !d.isPrimary) ?? displays[1]
    if (second) {
    ...
    console.warn('[FDN SW] moveHotelPolicyToGuestDisplay failed:', e)
  }
}

/** Open (or focus) the built-in hotel policy on the guest-facing display. */
async function openHotelPolicyWindow(): Promise<ExtensionResponse> {
  const policyUrl = chrome.runtime.getURL('policy-display.html')

  if (hotelPolicyWindowId != null) {
    try {
      const existing = await chrome.windows.get(hotelPolicyWindowId, { populate: true })
      if (existing?.id != null) {
        await moveHotelPolicyToGuestDisplay(existing.id)
        return { ok: true }
      }
    } catch {
      hotelPolicyWindowId = null
  ...
  try {
    ...
    if (!win?.id) {
      return { ok: false, error: 'Could not create hotel policy window.' }
    }
    hotelPolicyWindowId = win.id
    await moveHotelPolicyToGuestDisplay(win.id)
    return { ok: true }
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : 'Failed to open hotel policy.' }
  }
}
...
function broadcastScanSideResult(side: 'front' | 'back', imageBase64: string): void {
  void (async () => {
    try {
      await writeScanSideToStorage(side, imageBase64)
      await chrome.storage.local.remove(['fdn_last_native_scan', 'lastScanResult'])
    } catch (e) {
      console.warn(`[FDN ID scan] ${side} image storage failed`, e)
    }
    const msg: ScanSideBroadcast = {
...
async function handleThalesNativeScan(payload: NativeScanSuccessPayload) {
  ...
  if (b64Front && !b64Back) {
    broadcastScanSideResult('front', b64Front)
    return
  }

  if (b64Back && !b64Front) {
    broadcastScanSideResult('back', b64Back)
    return
  }

  if (!isCompleteTwoSidedScan(b64Front, b64Back)) {
    console.warn('[FDN ID scan] ignored — incomplete two-sided scan payload')
    return
  }
...
async function fetchLatestIdScanForConfirmation(confirmationNumber: string): Promise<ExtensionResponse> {
  lastError = null
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return { ok: false, error: 'Not signed in' }
  ...
  if (!conf) return { ok: true, idScanLog: [] }
  ...
  if (error) {
    console.warn('[FDN] latest id_scan by confirmation', error.message)
    return { ok: false, error: error.message }
  }
...
function isFrontDeskEmergencyGrantActive(
  role: string | null,
  settings: Awaited<ReturnType<typeof loadExtensionHotelSettings>>,
  now = new Date(),
): boolean {
  if (role !== 'front_desk') return false
  const until = settings.frontDeskKeysWriteAccessUntil
...
async function requireKeysWriteAccess(
  ...
  frontDeskEmergencyWrite?: boolean,
): Promise<ExtensionResponse | null> {
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return { ok: false, error: 'Not signed in' }
  // Front desk (and above) may re-encode an occupied room without manager PIN.
  // Housekeeper is excluded — they should not encode keys at all.
  if (frontDeskOccupied && cachedRole && cachedRole !== 'housekeeper') return null
  const settings = await loadExtensionHotelSettings(client)
  if (frontDeskEmergencyWrite && isFrontDeskEmergencyGrantActive(cachedRole, settings)) return null
...
async function handleKeysAdminEncode(
  ...
  const denied = await requireKeysWriteAccess(msg.managerPin, msg.frontDeskOccupied)
  if (denied) return denied
...
async function keysBoardWriteContext(): Promise<
  | { ok: false; error: string }
  | { ok: true; client: SupabaseClient; userId: string; username: string | null; role: string | null }
> {
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  const user = sess.session?.user
...
async function handleKeysAddGuest(
  msg: Extract<ExtensionMessage, { type: 'KEYS_ADD_GUEST' }>,
): Promise<ExtensionResponse> {
  const denied = await requireKeysWriteAccess(msg.managerPin, false, true)
  if (denied) return denied

  const ctx = await keysBoardWriteContext()
  if (!ctx.ok) return { ok: false, error: (ctx as { error: string }).error }

  const prep = await keysAddGuestSw(
    ctx.client,
    {
      roomNumber: msg.roomNumber,
      guestName: msg.guestName,
  ...
  const result = await runRfidMakeKey({
    ...
    checkoutTime: msg.checkoutTime,
    cardSerial: 1,
    confirmationNumber: prep.confirmationNumber!,
    guestName: msg.guestName,
    portalAdminEncode: true,
  })
  return result as ExtensionResponse
...
async function handleKeysMoveRoom(
  msg: Extract<ExtensionMessage, { type: 'KEYS_MOVE_ROOM' }>,
): Promise<ExtensionResponse> {
  const denied = await requireKeysWriteAccess(msg.managerPin, false, true)
  if (denied) return denied
  ...
  const prep = await keysMoveRoomSw(
    ctx.client,
    {
      fromRoom: msg.fromRoom,
      toRoom: msg.toRoom,
      confirmationNumber: msg.confirmationNumber,
      guestName: msg.guestName,
      businessDate: localDateString(),
  ...
  const encoded = result as ExtensionResponse
  if (!encoded.ok) return encoded
  return {
    ...encoded,
    keyReminder: prep.keyReminder,
    movedToRoom: msg.toRoom,
  }
}

async function handleKeysRemoveGuest(
  msg: Extract<ExtensionMessage, { type: 'KEYS_REMOVE_GUEST' }>,
): Promise<ExtensionResponse> {
  const denied = await requireKeysWriteAccess(msg.managerPin)
  if (denied) return denied

  const ctx = await keysBoardWriteContext()
  if (ctx.ok === false) return { ok: false, error: ctx.error }

  const prep = await keysRemoveGuestSw(
    ctx.client,
    {
      roomNumber: msg.roomNumber,
      confirmationNumber: msg.confirmationNumber,
  ...
  if (prep.error) return { ok: false, error: prep.error }
  return { ok: true, keyReminder: prep.keyReminder }
}
...
async function fetchPmsBoardDataMsg(): Promise<ExtensionResponse> {
  lastError = null
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return { ok: false, error: 'Not signed in' }
  try {
    const pmsBoardData = await fetchPmsBoardData(client)
    return { ok: true, pmsBoardData }
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Could not load Dual PMS board'
    return { ok: false, error: message }
  }
}

async function triggerPmsSyncMsg(): Promise<ExtensionResponse> {
  lastError = null
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return { ok: false, error: 'Not signed in' }
  try {
    const pmsSyncResult = await triggerPmsSync(client)
    return { ok: true, pmsSyncResult }
  } catch (e) {
    const message = e instanceof Error ? e.message : 'PMS sync failed'
    return { ok: false, error: message }
  }
}

async function requestPmsHousekeepingMsg(
  roomNumbers: string[],
  status: 'clean' | 'dirty',
): Promise<ExtensionResponse & { hkMessage?: string }> {
  lastError = null
  const client = getClient()
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return { ok: false, error: 'Not signed in' }
  try {
    const result = await requestPmsHousekeeping(client, { roomNumbers, status })
    if (!result.ok || result.error) {
      return { ok: false, error: result.error ?? 'Housekeeping request failed' }
    }
    return { ok: true, hkMessage: result.message ?? undefined }
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Housekeeping request failed'
    return { ok: false, error: message }
  }
}

async function fetchSeniorRoomRecommendationsMsg(
  businessDate?: string,
  ...
  const { data: sess } = await client.auth.getSession()
  if (!sess.session) return { ok: false, error: 'Not signed in' }
  const hotelSettings = await loadExtensionHotelSettings(client)
  if (!hotelSettings.seniorRecommendEnabled || hotelSettings.seniorRecommendAge <= 0) {
    return {
      ok: true,
      seniorRoomRecommendations: {
        recommended: [],
        groups: [],
        floor: null,
        sameFloorCluster: false,
        clusterQuality: 'none',
        notes: [],
        needsGuestConfirmation: false,
        fallback: [],
        usedCustomList: false,
        preferredFloors: hotelSettings.seniorPreferredFloors,
        busyCount: 0,
        propertyLayoutId: null,
      },
    }
  }
  try {
    const date = businessDate?.trim() || localDateString()
    const seniorRoomRecommendations = await fetchSeniorRoomRecommendations(client, date, {
      seniorRecommendEnabled: hotelSettings.seniorRecommendEnabled,
      seniorRecommendAge: hotelSettings.seniorRecommendAge,
      seniorPreferredFloors: hotelSettings.seniorPreferredFloors,
      seniorPreferredRoomList: hotelSettings.seniorPreferredRoomList,
    }, roomRequests)
    return { ok: true, seniorRoomRecommendations }
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Could not load room recommendations'
    return { ok: false, error: message }
  }
}
...
async function handleMessage(
  ...
  if (msg.type === 'REFRESH_BALANCE') {
    if (!reservation?.confirmationNumber) {
      return { ok: false, reason: 'no_reservation' }
    }
    if (reservation.pms === 'synxis') {
      const accounting = await fetchSynxisAccountingSummary(reservation.confirmationNumber)
      if (accounting && accounting.estimatedRemaining !== null) {
        reservation = { ...reservation, dueAmount: String(accounting.estimatedRemaining) }
      }
    } else if (reservation.pms === 'ezee') {
      const conf = reservation.confirmationNumber.trim()
      const detail = await fetchEzeeReservationDetailFromApi(conf)
      if (detail && ezeeGuestDisplay) {
        const tabUrl = reservation.pageUrl
        const loadedAt = new Date().toISOString()
        reservation = reservationFromEzeeReservationDetail(reservation, detail, tabUrl, loadedAt)
        ...
        const billing = await fetchEzeeAccountingSummary(conf)
        if (billing !== null) {
          reservation = { ...reservation, dueAmount: String(billing.dueAmount) }
        }
      }
  ...
  if (msg.type === 'GET_LATEST_ID_SCAN_FOR_CONFIRMATION') {
    return fetchLatestIdScanForConfirmation(msg.confirmationNumber)
  }
  ...
  if (msg.type === 'GET_PMS_BOARD_DATA') {
    return fetchPmsBoardDataMsg()
  }

  if (msg.type === 'TRIGGER_PMS_SYNC') {
    return triggerPmsSyncMsg()
  }

  if (msg.type === 'REQUEST_PMS_HOUSEKEEPING') {
    return requestPmsHousekeepingMsg(msg.roomNumbers, msg.status)
  }

  if (msg.type === 'GET_SENIOR_ROOM_RECOMMENDATIONS') {
    return fetchSeniorRoomRecommendationsMsg(msg.businessDate, msg.roomRequests)
  }
  ...
  if (msg.type === 'KEYS_ADD_GUEST') {
    return handleKeysAddGuest(msg)
  }

  if (msg.type === 'KEYS_MOVE_ROOM') {
    return handleKeysMoveRoom(msg)
  }

  if (msg.type === 'KEYS_REMOVE_GUEST') {
    return handleKeysRemoveGuest(msg)
  }
  ...
  if (msg.type === 'LOAD_EZEE_RESERVATION') {
    ...
    return completeEzeeReservationFromSnapshot(
      tab.id,
      tab.url ?? 'https://live.ipms247.com/',
      tr.snapshot,
      tr.guestDisplay,
  ...
  if (msg.type === 'WINDOW_CONTROL') {
    try {
      const result = await sendNativeRequest({ type: 'WINDOW_CONTROL', action: msg.action, processName: msg.processName })
  ...
  if (msg.type === 'OPEN_HOTEL_POLICY') {
    return openHotelPolicyWindow()
  }
  ...
  if (msg.type === 'EZEE_OPEN_REG_CARD') {
    console.info('[FDN SW] EZEE_OPEN_REG_CARD', {
      confirmation: msg.confirmation,
      url: msg.ezeeReportUrl.slice(0, 120),
    })
    const existingWinId = ezeeRegCardWindowByConf.get(msg.confirmation)
    if (existingWinId != null) {
      try {
        await chrome.windows.remove(existingWinId)
      } catch {
        /* already closed */
      }
    ...
    await chrome.storage.local.set({ [FDN_REG_CARD_SIGNING_ACTIVE_KEY]: true })
  ...
  if (msg.type === 'EZEE_SAVE_SIGNATURE') {
    ...
    try {
      ...
      if (result.ok) {
        await notifyRegCardSignatureComplete()
      }
      return result
    } catch (e) {
  ...
  if (msg.type === 'GET_MATCHING_RESERVATIONS') {
    ...
    const matched = await findReservationsByGuestName(client, msg.guestName, 20)
  ...
  if (msg.type === 'SELECT_EZEE_GROUP_MEMBER') {
    return selectEzeeGroupMember(msg.index)
  }
...
// When SynXis finishes loading in a tab, capture the latest cookies as a fallback.
chrome.tabs.onUpdated.addListener((_tabId, info, tab) => {
  if (info.status !== 'complete') return
  const u = tab?.url ?? null
  if (!isSynxisUrl(u)) return
  void captureAndSaveSynxisCookie('tab_complete')
})
...
chrome.runtime.onMessageExternal.addListener((message, _sender, sendResponse) => {
  void (async () => {
    try {
      ...
      if (m.channel === PORTAL_BRIDGE_CHANNEL && m.type === 'OPEN_HOTEL_POLICY') {
        sendResponse(await handleMessage({ type: 'OPEN_HOTEL_POLICY' }, noopSender))
        return
      }
...
setInterval(() => { void refreshHardwareCache() }, HW_STATUS_TTL_MS)
